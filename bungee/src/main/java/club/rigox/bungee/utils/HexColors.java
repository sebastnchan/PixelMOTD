...
public class HexColors {
    ...
    private static final Pattern GRADIENT = Pattern.compile("<gradient: #([A-Fa-f0-9]){6}-#([A-Fa-f0-9]){6}:[^%]*>");
    ...
    public String applyColor(String message) {
        if (hexStatus) {
            ...
            Matcher normalHex   = HEX.matcher(message);
            Matcher gradientHex = GRADIENT.matcher(message);
            ...
            while (gradientHex.find()) {
                ...
                for (double i = 0.0D; i < 1.0D; i += l) {
                    ...
                    part.append(ChatColor.of(String.format("#%02X%02X%02X", r, g, b)).toString())
                            .append(code1)
                            .append(code2)
                            .append(code3)
                            .append(code4)
                            .append(code5)
                            .append(gradientTextLetters[join]);
                    join++;
                }
    ...
    public void setHex(boolean toggleHex) {
        hexStatus = toggleHex;
    }
